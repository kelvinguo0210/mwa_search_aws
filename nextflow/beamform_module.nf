...
process make_beam {
    ...
    //container {container}
    //container 'cirapulsarsandtransients/vcstools:master'
...
-d ${params.scratch_basedir}/${params.obsid} -P ${point.join(",").replaceAll(~/\s/,"")} \
-r 10000 -m ${params.scratch_basedir}/${params.obsid}/cal/${params.obsid}_metafits_ppds.fits \
...
workflow pre_beamform {
    main:
        beamform_setup()
        ////kguo combined_data_check(beamform_setup.out[0].splitCsv())
    emit:
